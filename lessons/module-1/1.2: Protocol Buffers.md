...
Test your understanding:
   ...
   <details><summary>What's the purpose of field numbers in protobuf?</summary>Field numbers are used for serialization and must remain constant for backward compatibility. They identify fields in the binary format.</details>


   <details><summary>What's the difference between `repeated` and `map`?</summary>`repeated` creates an array/list of values, while `map` creates key-value pairs. Maps are more efficient for lookups by key.</details>


   <details><summary>Why should enum values always include a zero value?</summary>Proto3 requires a zero value as the default. It's also a best practice for backward compatibility and clear intention.</details>


   <details><summary>When would you use `oneof` instead of optional fields?</summary>Use `oneof` when you have mutually exclusive options - only one field should be set at a time, like different payment methods.</details>
